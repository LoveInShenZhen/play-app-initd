...
IPADDR=0.0.0.0
APP_DIR=
APP_CONF=${APP_DIR}/conf/application.conf
LOGBACK_CONF=${APP_DIR}/conf/logback.xml

# Path to the RUNNING_PID file containing process ID
PID_FILE=$APP_DIR/RUNNING_PID

#  JVM arguments , default JVM settings will be used: -J-Xms128M -J-Xmx512m -J-server
JVM_ARGS="-Xms1024m -Xmx2048m -XX:ReservedCodeCacheSize=128m -server"

# ***********************************************
# ***********************************************

# Additional arguments to be passed to the Play application
APP_ARGS="${JVM_ARGS} -Duser.dir=${APP_DIR} -Dhttp.port=${PORT} -Dhttp.address=${IPADDR} -Dconfig.file=${APP_CONF} -Dlogger.file=${LOGBACK_CONF} -Dpidfile.path=${PID_FILE}"
...
checkAppStarted()
{
        ...
        COUNTER=0
        while [ "$HTTP_RESPONSE_CODE" != "200" ] && [ $COUNTER -lt 10 ]; do
                sleep 1
...
case "${COMMAND}" in
    start)
        ...
        PID=`$SCRIPT_TO_RUN $APP_ARGS > $TMP_LOG & echo $!`
